...
class Minify_Controller_Version1 extends Minify_Controller_Base {
    ...
    public function setupSources($options) {
        ...
        if (Params::existParam('files')) {
            Params::setParam('files', str_replace("\x00", '', (string)Params::getParam('files')));
        }
        ...
        if (!Params::existParam('files')
            ...
            || ! preg_match('/^[^,]+\\.(css|js)(,[^,]+\\.\\1)*$/', Params::getParam('files'), $m)
            // no "//" (makes URL rewriting easier)
            || strpos(Params::getParam('files'), '//') !== false
            // no "\"
            || strpos(Params::getParam('files'), '\\') !== false
            // no "./"
            || preg_match('/(?:^|[^\\.])\\.\\//', Params::getParam('files'))
        ) {
            return $options;
        }
