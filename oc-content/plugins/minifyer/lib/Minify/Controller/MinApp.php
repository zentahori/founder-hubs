    ...
    public function setupSources($options) {
        ...
        foreach (array('g', 'b', 'f') as $key) {
            if (Params::existParam($key)) {
                Params::setParam($key, str_replace("\x00", '', (string)Params::getParam($key)));
            }
        }
        ...
        if (! $cOptions['groupsOnly'] && Params::existParam('f')) {
            ...
            if (// verify at least one file, files are single comma separated, 
                ...
                ! preg_match('/^[^,]+\\.(css|js)(?:,[^,]+\\.\\1)*$/', Params::getParam('f'), $m)
                // no "//"
                || strpos(Params::getParam('f'), '//') !== false
                // no "\"
                || strpos(Params::getParam('f'), '\\') !== false
            ) {
            ...
            if (Params::existParam('b')) {
                // check for validity
                if (preg_match('@^[^/]+(?:/[^/]+)*$@', Params::getParam('b'))
                    && false === strpos(Params::getParam('b'), '..')
                    && Params::getParam('b') !== '.') {
                    // valid base
